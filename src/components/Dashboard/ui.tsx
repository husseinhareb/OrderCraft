...
export const MiniLine: FC<{
  ...
  const sy = (y: number) =>
    pad + (h - pad * 2) * (1 - (y - minY) / (maxY - minY));
  const dStr = data
...
export const MiniBars: FC<{
  ...
  useEffect(() => {
    ...
    const compute = () => {
      ...
      const desired =
        data.length * minColPx + gap * Math.max(0, data.length - 1);
...
export const Heatmap: React.FC<{
  ...
  const cellW = Math.max(
    8,
    Math.floor((width - labelCol - gap * (cols - 1)) / 24)
  );
  const cellH = Math.max(
    8,
    Math.floor((height - headerHeight - gap * (rows - 1)) / 7)
  );
